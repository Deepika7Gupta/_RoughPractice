...
results1=cross_val_score(model,X,Y,cv=kfold,scoring='neg_log_loss')
...
print('roc acu is: %.3f(%.3f)'%(results2.mean(),results2.std()))
...
import pandas as pd
datar=pd.read_excel('E:\\ML\\Coding Assessment\\BostonHousing.xls')
array=datar.values
X1=array[:,0:13]
Y1=array[:,13]
...
resultr=cross_val_score(modelr,X1,Y1,cv=kfold,scoring='neg_mean_absolute_error')
print('mae: %.3f (%.3f)'%(resultr.mean(),resultr.std()))


# In[71]:


resultsr1=cross_val_score(modelr,X1,Y1,cv=kfold,scoring='neg_mean_squared_error')


# In[72]:


print('mse: %.3f (%.3f)'%(resultsr1.mean(),resultsr1.std()))


# In[73]:


resultsr2=cross_val_score(modelr,X1,Y1,cv=kfold,scoring='r2')


# In[74]:


print('m2: %.3f (%.3f)'%(resultsr2.mean(),resultsr2.std()))
...
result=cross_val_score(model,X1,Y1,cv=kfold,scoring='neg_mean_squared_error')
result.mean()
